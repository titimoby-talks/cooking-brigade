...
public class BrigadeChatCLI {
    ...
    private void preloadSession(String id) {
        try {
            var req = HttpRequest.newBuilder()
                    .uri(URI.create(API_BASE + "/sessions/" + id + "/history"))
                    .version(HttpClient.Version.HTTP_1_1)
            ...
            if (resp.statusCode() == 200) {
                var arr = JsonParser.parseString(resp.body())
                        .getAsJsonObject().get("history").getAsJsonArray();
                for (var item : arr) {
                ...
                System.err.println("Session introuvable : " + id);
                System.exit(1);
            }
        } catch (Exception e) {
            System.err.println("Erreur lors de la reprise de session : " + e.getMessage());
            System.exit(1);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length - 1; i++) {
            if ("--session".equals(args[i])) {
                app.preloadSession(args[i + 1]);
            }
        }

        // Thème CSS
        StyleEngine styleEngine = StyleEngine.create();
        styleEngine.addStylesheet("brigade", """
            Panel {
                border-type: rounded;
            }
            """);
        styleEngine.setActiveStylesheet("brigade");

        app.runner = ToolkitRunner.create();
        app.runner.styleEngine(styleEngine);
